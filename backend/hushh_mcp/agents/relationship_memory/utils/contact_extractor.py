...
class ContactExtractor:
    ...
    def __init__(self):
        ...
        self.name_patterns = [
            ...
            re.compile(r'(?:add|create|new)\s+(?:a\s+)?contact\s+(?:named?\s+)?([A-Za-z]+(?:\s+[A-Za-z]+)*)', re.IGNORECASE),
            # Pattern for "contact name X"
            re.compile(r'contact\s+name\s+([A-Za-z]+(?:\s+[A-Za-z]+)*)', re.IGNORECASE),
            # Pattern for "I met X" or "met X"
            re.compile(r'(?:met|meet)\s+([A-Za-z]+(?:\s+[A-Za-z]+)*)', re.IGNORECASE),
            # Pattern for "X with email" (name before email context)
            re.compile(r'([A-Za-z]+(?:\s+[A-Za-z]+)*)\s+with\s+email', re.IGNORECASE),
            # Pattern for "contact X"
            re.compile(r'contact\s+([A-Za-z]+(?:\s+[A-Za-z]+)*)', re.IGNORECASE),
